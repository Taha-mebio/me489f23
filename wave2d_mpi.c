...
double elapsed=mpiEndTimer(start);
    if(rank==0){
      printf("with %d processes taken, the program took %g seconds\n", size, elapsed);
    }
  printf("Infinity norm of the error: %.4e %.8e \n", linf, time);
  MPI_Finalize();
  free(q0);free(q1);free(qn); free(x); free(y);
  return 0;
}
